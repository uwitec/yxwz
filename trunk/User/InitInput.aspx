...
<asp:Content ID="Content1" ContentPlaceHolderID="main" Runat="Server">
  <script language=javascript type="text/jscript">
    ...
    <asp:SqlDataSource ID="psjl" runat="server" ConflictDetection="CompareAllValues"
        ...
        SelectCommand="SELECT 领货记录.领取用户, 领货记录.领取数量, 材料价格.材料名称, 材料价格.型号, 领货记录.id FROM 领货记录 INNER JOIN 材料价格 ON 领货记录.材料id = 材料价格.id WHERE (领货记录.备注 = '仓库初始化录入') order by id desc" DeleteCommand="DELETE FROM 领货记录 WHERE (id = @original_id)">
        <InsertParameters>
